function qmcscatter!(ax::GLMakie.Axis,x::Matrix{Float64},nvec::Vector{Int64})
    @assert size(x,1)>=maximum(nvec) && size(x,2)==2
    nprev = 1
    for i in eachindex(nvec)
        n = nvec[i]
        GLMakie.scatter!(ax,x[nprev:n,1],x[nprev:n,2])
        nprev = n+1
    end
end
...
function qmcscatter!(xs::Vector{Matrix{Float64}},nvec::Vector{Int64},dvec::Matrix{Int64}=[1 2;])
    ...
    fig = GLMakie.Figure(backgroundcolor=:white, resolution=(400*ncols,400*nrows))
    for i=1:nrows
        for j=1:ncols
            dhoriz,dvert = dvec[i,1],dvec[i,2]
            x = xs[j][:,[dhoriz,dvert]]
            ax = GLMakie.Axis(fig[i,j],
                xlabel = L"$X_{"*"$dhoriz"*L"}$",
                ylabel = L"$X_{"*"$dvert"*L"}$",
                xticks = ([0,1/4,1/2,3/4,1],[L"$0$",L"$1/4$",L"$1/2$",L"$3/4$",L"$1$"]),
                yticks = ([0,1/4,1/2,3/4,1],[L"$0$",L"$1/4$",L"$1/2$",L"$3/4$",L"$1$"]))
            GLMakie.limits!(ax,[0,1],[0,1])
